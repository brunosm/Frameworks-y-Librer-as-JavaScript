$(document).ready(function() {
  ...
  //---- Funcion para agregar dulces ------ //
  function nuevosdulces(columna){

    for(var m= 1; m<8; m++){
        if($(".col-"+m+" > img").length<7){
          var faltantes = 8 - $(".col-"+m+" > img").length;

          for(var n=1;n<faltantes;n++)
          {
            if($(".col-"+m).children("img:nth-child("+i+")").html()==null)
            {
              numero=Math.floor(Math.random() * 4) + 1 ;
              imagen="image/"+numero+".png";
